...
// ─── EXEC CMD API — ينفّذ الأمر داخلياً مباشرةً دون المرور عبر Messenger ─────
app.post("/api/exec-cmd", auth, async (req, res) => {
  try {
    const { threadID, cmd } = req.body;
    if (!threadID || !cmd) return res.json({ error: "threadID و cmd مطلوبان" });

    const fca = global.GoatBot?.fcaApi;
    if (!fca) return res.json({ error: "البوت غير متصل" });

    // Find command registry
    const commandsMap = global.GoatBot?.commands || global.GoatBot?.cmds || new Map();

    // Parse prefix + command name + args
    const prefix = global.GoatBot?.config?.prefix || "/";
    const body   = cmd.startsWith(prefix) ? cmd : (prefix + cmd);
    ...
    if (commandsMap instanceof Map) {
      commandModule = commandsMap.get(cmdName);
      if (!commandModule) {
        for (const [, mod] of commandsMap) {
          const aliases = mod?.config?.aliases || [];
          if (aliases.map(a => String(a).toLowerCase()).includes(cmdName)) {
            commandModule = mod; break;
          }
        }
      }
    }
    ...
    if (!commandModule || typeof commandModule.onStart !== "function") {
      // Fall back to sending as message if command not found in registry
      return new Promise((resolve) => {
        let replied = false;
        const timer = setTimeout(() => {
          if (!replied) { replied = true; resolve(res.json({ ok: true, method: "message", warn: "الأمر غير موجود في السجل — أُرسل كرسالة" })); }
        }, 10000);
        fca.sendMessage(cmd, threadID, (err) => {
          if (replied) return; replied = true; clearTimeout(timer);
          if (err) return resolve(res.json({ error: err.message || String(err) }));
          resolve(res.json({ ok: true, method: "message" }));
        });
      });
    }

    // Get an admin ID to act as sender
    const adminIDs = global.GoatBot?.config?.adminBot || global.GoatBot?.config?.superAdminBot || [];
    const senderID = String(adminIDs[0] || fca.getCurrentUserID() || "0");

    // Build fake message helper
    const replies = [];
    const fakeMessage = {
      reply: async (text) => {
        replies.push(typeof text === "object" ? text.body || JSON.stringify(text) : String(text));
        try {
          const payload = typeof text === "object" ? text : { body: String(text) };
          await new Promise((ok, fail) => fca.sendMessage(payload, threadID, (e) => e ? fail(e) : ok()));
        } catch (_) {}
        return { messageID: "panel_" + Date.now() };
      },
      react: async (emoji) => {
        try { await new Promise((ok) => fca.setMessageReaction(emoji, "fake_mid_" + Date.now(), ok, true)); } catch (_) {}
      },
      unsend: async () => {}
    };
    ...
    const fakeEvent = {
      ...
      messageID:  "panel_exec_" + Date.now(),
      body,
      type:       "message",
      isGroup:    true,
      attachments: [],
    ...
    res.json({ ok: true, method: "exec", replies });
  } catch (e) { res.json({ error: e.message }); }
});
...
<div style="display:flex;flex-direction:column;gap:16px">
  ...
  <div class="card" style="border-color:rgba(99,102,241,.35)">
    ...
    <!-- Angel Controls -->
    <div style="margin-bottom:14px;background:var(--bg3);border:1px solid rgba(251,191,36,.25);border-radius:10px;padding:12px">
      <div style="font-size:.78rem;font-weight:700;color:#fbbf24;margin-bottom:9px;display:flex;align-items:center;gap:6px">
        <span>😇 Angel — الإرسال التلقائي (بالثواني)</span>
        <button class="btn btn-outline btn-sm" onclick="sendCmd('/angel status')" style="margin-right:auto;font-size:.68rem;padding:2px 8px">📊 حالة</button>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-bottom:8px">
        <button class="btn btn-sm" onclick="sendCmd('/angel on')" style="background:rgba(251,191,36,.18);color:#fbbf24;border:1px solid rgba(251,191,36,.4);font-weight:700">▶️ تشغيل</button>
        <button class="btn btn-outline btn-sm" onclick="sendCmd('/angel off')" style="border-color:rgba(239,68,68,.4);color:#f87171;font-weight:700">⏹️ إيقاف</button>
      </div>
      <div style="display:flex;gap:7px;align-items:center;margin-bottom:7px">
        <input type="text" id="angelMsg" class="form-control" placeholder="الرسالة التلقائية..." style="flex:1;margin:0;font-size:.8rem"/>
        <button class="btn btn-outline btn-sm" onclick="sendCmdWithInput('/angel change ','angelMsg')" style="white-space:nowrap;color:#fbbf24;border-color:rgba(251,191,36,.4)">✏️ تعديل</button>
      </div>
      <div style="display:flex;gap:7px;align-items:center">
        <input type="number" id="angelTimeMin" class="form-control" placeholder="أدنى (ث)" min="1" style="flex:1;margin:0;font-size:.8rem"/>
        <input type="number" id="angelTimeMax" class="form-control" placeholder="أقصى (ث)" min="1" style="flex:1;margin:0;font-size:.8rem"/>
        <button class="btn btn-outline btn-sm" onclick="sendAngelTime()" style="white-space:nowrap;color:#60a5fa;border-color:rgba(96,165,250,.4)">⏱️ وقت</button>
      </div>
    </div>

    <!-- Divel Controls -->
    <div style="margin-bottom:12px;background:var(--bg3);border:1px solid rgba(139,92,246,.25);border-radius:10px;padding:12px">
      <div style="font-size:.78rem;font-weight:700;color:#a78bfa;margin-bottom:9px;display:flex;align-items:center;gap:6px">
        <span>😈 Divel — رد بعد الصمت (بالثواني)</span>
        <button class="btn btn-outline btn-sm" onclick="sendCmd('/divel status')" style="margin-right:auto;font-size:.68rem;padding:2px 8px">📊 حالة</button>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-bottom:8px">
        <button class="btn btn-sm" onclick="sendCmd('/divel on')" style="background:rgba(139,92,246,.18);color:#a78bfa;border:1px solid rgba(139,92,246,.4);font-weight:700">▶️ تشغيل</button>
        <button class="btn btn-outline btn-sm" onclick="sendCmd('/divel off')" style="border-color:rgba(239,68,68,.4);color:#f87171;font-weight:700">⏹️ إيقاف</button>
      </div>
      <div style="display:flex;gap:7px;align-items:center;margin-bottom:7px">
        <input type="text" id="divelMsg" class="form-control" placeholder="رسالة الرد التلقائي..." style="flex:1;margin:0;font-size:.8rem"/>
        <button class="btn btn-outline btn-sm" onclick="sendCmdWithInput('/divel change ','divelMsg')" style="white-space:nowrap;color:#a78bfa;border-color:rgba(139,92,246,.4)">✏️ تعديل</button>
      </div>
      <div style="display:flex;gap:7px;align-items:center">
        <input type="number" id="divelTimeMin" class="form-control" placeholder="أدنى (ث)" min="1" style="flex:1;margin:0;font-size:.8rem"/>
        <input type="number" id="divelTimeMax" class="form-control" placeholder="أقصى (ث)" min="1" style="flex:1;margin:0;font-size:.8rem"/>
        <button class="btn btn-outline btn-sm" onclick="sendDivelTime()" style="white-space:nowrap;color:#60a5fa;border-color:rgba(96,165,250,.4)">⏱️ وقت</button>
      </div>
    </div>
...
<div style="display:flex;flex-direction:column;gap:16px">
  ...
  <div class="card" style="padding:0;overflow:hidden;flex:1">
    <div style="padding:14px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between">
      ...
      <div style="display:flex;gap:8px;align-items:center">
        <span id="feedLiveSpan" style="display:none;font-size:.7rem;color:var(--green);font-weight:700;align-items:center;gap:4px"><span style="width:7px;height:7px;border-radius:50%;background:var(--green);display:inline-block;animation:pulse 1.2s infinite"></span>مباشر</span>
...
async function sendCmd(cmd) {
  ...
  st.innerHTML = \`<span style="color:var(--text3)">⏳ جارٍ تنفيذ: \${escHtmlJS(cmd)}</span>\`;
  // Try exec-cmd first (direct internal execution), fall back to send
  let r = await api('/api/exec-cmd', { threadID, cmd });
  if (!r.ok) {
    r = await api('/api/send', { threadID, message: cmd });
  }
  if (r.ok) {
    const method = r.method === 'exec' ? '⚡ نُفِّذ مباشرة' : '📤 أُرسل';
    st.innerHTML = \`<span style="color:var(--green)">\${method}: \${escHtmlJS(cmd)}</span>\`;
    showToast('✅ ' + method + ': ' + cmd, 'success');
  } else {
...
async function sendAngelTime() {
  const min = document.getElementById('angelTimeMin').value.trim();
  const max = document.getElementById('angelTimeMax').value.trim();
  if (!min) return showToast('❌ أدخل قيمة أدنى على الأقل', 'error');
  const cmd = max && max !== min ? \`/angel time \${min} \${max}\` : \`/angel time \${min}\`;
  await sendCmd(cmd);
}

async function sendDivelTime() {
  const min = document.getElementById('divelTimeMin').value.trim();
  const max = document.getElementById('divelTimeMax').value.trim();
  if (!min) return showToast('❌ أدخل قيمة أدنى على الأقل', 'error');
  const cmd = max && max !== min ? \`/divel time \${min} \${max}\` : \`/divel time \${min}\`;
  await sendCmd(cmd);
}
...
async function checkBotStatus() {
  try {
    const r = await fetch('/api/status');
    const d = await r.json();
    if (d.botID) window._botID = d.botID;
