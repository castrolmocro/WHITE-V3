...
/** Builds automatic context: bot structure summary + sample command files */
...
adminBot IDs: ${(cfg.adminBot || []).join(", ")}
...
Node: ${pkg.engines?.node || "—"}`);
  ...
  // 5. Sample command file (show one full command so AI knows the exact format)
  try {
    const sampleCmds = ["nm.js", "angel.js", "divel.js"];
    for (const sampleFile of sampleCmds) {
      const samplePath = path.join(ROOT, "scripts/cmds", sampleFile);
      if (fs.existsSync(samplePath)) {
        const content = fs.readFileSync(samplePath, "utf8").slice(0, 4000);
        parts.push(`=== مثال أمر حقيقي (${sampleFile}) — القالب المطلوب ===\n${content}`);
        break;
      }
    }
  } catch (_) {}

  // 6. config.json full (for AI to understand full bot config)
  try {
    const cfgRaw = fs.readFileSync(path.join(ROOT, "config.json"), "utf8").slice(0, 2000);
    parts.push(`=== config.json الكامل ===\n${cfgRaw}`);
  } catch (_) {}
...
async function callAI(model, messages) {
  ...
  // Only models confirmed working on pollinations.ai (anonymous tier)
  const WORKING_MODELS = ["openai-fast", "openai"];
  const preferred = WORKING_MODELS.includes(model) ? model : "openai-fast";
  const order = [preferred, ...WORKING_MODELS.filter(m => m !== preferred)];

  let lastErr = "";

  // ── Strategy 1: Chat API with working models ──────────────────────────────
  for (const mdl of order) {
    ...
    try {
      const res = await fetch("https://text.pollinations.ai/openai", {
        ...
        body: JSON.stringify({ model: mdl, messages, stream: false, seed: Math.floor(Math.random() * 99999) })
      });
      clearTimeout(timer);
      ...
      clearTimeout(timer);
      lastErr = e.name === "AbortError" ? `انتهت مهلة ${mdl}` : e.message;
    }
  }

  // ── Strategy 2: Simple text GET fallback ──────────────────────────────────
  const userMsg = messages.filter(m => m.role !== "system").map(m => m.content).join("\n").slice(0, 500);
  const ctrl2 = new AbortController();
  const t2 = setTimeout(() => ctrl2.abort(), 25000);
  try {
    const r2 = await fetch(`https://text.pollinations.ai/${encodeURIComponent(userMsg)}?model=openai&seed=${Math.floor(Math.random()*9999)}`, {
      signal: ctrl2.signal, headers: { "User-Agent": "WHITE-V3-DevHub/3.0" }
    });
    clearTimeout(t2);
    if (r2.ok) {
    ...
    lastErr = `text-fallback HTTP ${r2.status}`;
  } catch (e) {
    clearTimeout(t2);
    lastErr = e.name === "AbortError" ? "انتهت مهلة fallback" : e.message;
  }
...
=== القالب الصحيح لأوامر WHITE V3 ===
module.exports = {
  config: { name, version, author, countDown, role, description, category, guide },
  onStart: async ({ api, event, args, message, threadsData, usersData }) => {},
  onEvent: async (...) => {},   // اختياري - يُستدعى عند أحداث الغروب
  onChat:  async (...) => {}    // اختياري - يُستدعى عند كل رسالة
};
role: 0=الجميع  1=مشرف_المجموعة  2=مشرف_البوت

=== أنظمة الحماية المهمة ===
- التحقق من admin: global.GoatBot.config.adminBot.includes(String(senderID))
- قاعدة البيانات: threadsData.get/set(threadID, "data.key")
- API البوت: global.GoatBot.fcaApi

=== قواعد التحليل ===
- اقرأ الطلب وحدد: أمر جديد؟ تعديل؟ إصلاح؟ سؤال؟
- اذكر الملفات المتأثرة
...
module.exports = {
  config: {
    ...
    author:      "WHITE V3",
    countDown:   3,
    role:        0,    // 0=الجميع  1=مشرف_غروب  2=مشرف_البوت
    description: "وصف الأمر",
    category:    "التصنيف",
    guide: { en: "  {pn} [مثال] — شرح" }
  },

  onStart: async function ({ api, event, args, message, threadsData, usersData }) {
    const { senderID, threadID, messageID } = event;

    // التحقق من صلاحيات مشرف البوت
    const adminBot = global.GoatBot.config.adminBot || [];
    if (!adminBot.includes(String(senderID))) return;

    // الرد
    return message.reply("نص الرد");
    // أو: await api.sendMessage("نص", threadID);
    // أو: await api.sendMessage({ body: "نص", attachment: stream }, threadID);
  }
};
\`\`\`

=== قواعد الكود ===
- استخدم module.exports = { config, onStart } وليس module.exports.config = {}
- معالجة الأخطاء بـ try/catch دائماً
- message.reply("نص") أو message.react("✅") للردود السريعة
- threadsData.get(threadID, "data.key") لقراءة بيانات الغروب
- threadsData.set(threadID, value, "data.key") لحفظ البيانات
- لا تستخدم require() إلا للمكتبات الأساسية (fs-extra, path)
- global.GoatBot.fcaApi للوصول للـ API خارج onStart
...
=== ما تتحقق منه ===
1. هل يستخدم module.exports = { config, onStart } وليس module.exports.config = {} ؟
2. هل config يحتوي: name, version, author, countDown, role, description, category, guide ؟
3. هل role صحيح؟ (0/1/2 — وليس hasPermssion)
4. هل onStart يستخدم { api, event, args, message, threadsData, usersData } ؟
5. هل هناك أخطاء syntax واضحة؟
6. هل يعالج الأخطاء بـ try/catch؟
7. هل يتعارض مع أي نظام حماية في البوت؟

=== صيغة الرد (إلزامية) ===
**الحكم:** ✅ جاهز للتطبيق  أو  ⚠️ يحتاج تعديل  أو  ❌ لا يعمل
**السبب:** جملة أو جملتان
**إن وجد مشكلة:** اذكر المشكلة بالضبط وكيف تُصلحها
**ملاحظة:** نصيحة مفيدة (اختياري)
...
async function saveEditedFile() {
  ...
  statusEl.innerHTML = '<span style="color:var(--text3)">⏳ جارٍ الحفظ...</span>';
  const r = await api("/api/devhub/file/write", {path: selectedEditFile, content});
  if (r.ok) {
    const hotMsg = r.hotReloaded ? ' <span style="color:#6ee7b7;font-size:.74rem">⚡ تم التحميل الفوري</span>' : '';
    showToast("✅ تم حفظ: " + selectedEditFile.split("/").pop(), "success");
    statusEl.innerHTML = \`<span style="color:var(--green)">✅ محفوظ — \${new Date().toLocaleTimeString("ar")}</span>\${hotMsg}\`;
    // رفع تلقائي لـ GitHub
    try {
      statusEl.innerHTML += ' <span style="color:var(--text3);font-size:.74rem">🐙 جارٍ رفع GitHub...</span>';
      ...
      if (pushData.ok) {
        showToast("🐙 GitHub محدَّث تلقائياً!", "success");
        statusEl.innerHTML = \`<span style="color:var(--green)">✅ محفوظ + رُفع لـ GitHub\${r.hotReloaded?" + تحميل فوري":""} — \${new Date().toLocaleTimeString("ar")}</span> <a href="\${pushData.url}" target="_blank" style="color:#60a5fa;font-size:.78rem">🐙 فتح</a>\`;
      } else {
        statusEl.innerHTML = \`<span style="color:var(--green)">✅ محفوظ محلياً\${hotMsg}</span> <span style="color:#fbbf24;font-size:.74rem">⚠️ GitHub: \${pushData.error||"تحقق من التوكن"}</span>\`;
      }
    } catch(e) {
      statusEl.innerHTML = \`<span style="color:var(--green)">✅ محفوظ محلياً\${hotMsg}</span> <span style="color:var(--text3);font-size:.74rem">⚠️ GitHub غير متاح</span>\`;
    }
  } else {
...
- لا تختصر الكود — اكتبه كاملاً`;
  ...
  app.post("/api/devhub/file/write", auth, (req, res) => {
    try {
      ...
      // Hot-reload: إعادة تحميل الأمر في الذاكرة مباشرةً دون إعادة تشغيل البوت
      let hotReloaded = false;
      if (p.startsWith("scripts/cmds/") && p.endsWith(".js")) {
        try {
          ...
          const cmdName = newModule?.config?.name;
          if (cmdName && global.GoatBot?.commands instanceof Map) {
            global.GoatBot.commands.set(cmdName, newModule);
            // Also re-register aliases
            const aliases = newModule?.config?.aliases || [];
            for (const alias of aliases) {
              global.GoatBot.commands.set(String(alias).toLowerCase(), newModule);
            }
            hotReloaded = true;
          }
        } catch (e2) {
          // Hot-reload failed but file was saved — not critical
        }
      } else if (p.startsWith("scripts/events/") && p.endsWith(".js")) {
        try {
          const fullPath = path.join(ROOT, p);
          delete require.cache[require.resolve(fullPath)];
          hotReloaded = false; // events need full restart — just note it
        } catch (_) {}
      }

      res.json({ ok: true, hotReloaded });
    } catch (e) { res.json({ error: e.message }); }
  });
...
<div class="gh-split">
  ...
  <div class="gh-editor-panel" id="ghEditorPanel">
    ...
    <div id="ghCommitRow" style="display:none;padding:6px 14px 8px;border-bottom:1px solid var(--border)">
      <div style="display:flex;gap:6px;align-items:center">
        <input type="text" id="ghCommitMsg" class="form-control" placeholder="رسالة الـ commit (اختياري)" style="flex:1;margin:0;font-size:.8rem"/>
        <button class="btn btn-success btn-sm" onclick="ghSaveConfirm()">✅ حفظ</button>
        <button class="btn btn-outline btn-sm" onclick="ghCancelSave()">✕</button>
      </div>
    </div>
...
function ghSaveFile() {
  ...
  const inp = document.getElementById('ghCommitMsg');
  inp.value = '✏️ تعديل: ' + _ghCurrent.split('/').pop();
  row.style.display = 'block';
  inp.focus(); inp.select();
}
function ghCancelSave() {
  document.getElementById('ghCommitRow').style.display = 'none';
}
async function ghSaveConfirm() {
  if (!_ghCurrent) return;
  const owner   = document.getElementById('ghfOwner').value.trim();
  const repo    = document.getElementById('ghfRepo').value.trim();
  const branch  = document.getElementById('ghfBranch').value.trim() || 'main';
...
async function ghSendAI() {
  ...
  if (!msg) return;
  const sendBtn = document.querySelector('#ghChatBox').closest('.gh-ai-panel').querySelector('button.btn-primary');
  ...
  box.insertAdjacentHTML('beforeend',\`<div class="gh-msg ai" id="ghAiStream"><span style="color:var(--text3)">⏳ AI يفكر... (قد يستغرق 20-60 ثانية)</span></div>\`);
  box.scrollTop = box.scrollHeight;
  const ctrl = new AbortController();
  ...
  try {
    ...
    if (el) {
      ...
      if (codeMatch) {
        _ghLastAI = codeMatch[1].trim();
        const applyBtn = \`<button class="btn btn-success btn-sm" style="margin-top:8px;width:100%" onclick="ghApplyAI()">✨ تطبيق الكود في المحرر</button>\`;
        el.innerHTML = reply.replace(/\`\`\`(?:javascript|js)?[\\r\\n]([\\s\\S]*?)\`\`\`/g,
          '<div style="background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:10px;margin:6px 0;font-family:monospace;font-size:.78rem;overflow-x:auto;white-space:pre">$1</div>') + applyBtn;
      } else {
        _ghLastAI = null;
    ...
    const el = document.getElementById('ghAiStream');
    const isTimeout = e.name === 'AbortError';
    if (el) el.innerHTML = \`<span style="color:var(--red)">❌ \${isTimeout ? 'انتهت مهلة الانتظار — حاول مرة أخرى' : ghEsc(e.message)}</span>\`;
  } finally {
...
function ghApplyAI(){
  if (!_ghLastAI) return showToast('اسأل AI أولاً وتأكد من أنه أعطاك كوداً','error');
  document.getElementById('ghfEditor').value = _ghLastAI;
  showToast('✅ تم تطبيق كود AI في المحرر — اضغط 💾 حفظ للرفع','success');
}
...
document.getElementById('ghCommitMsg').addEventListener('keydown', e => {
  if (e.key==='Enter') { e.preventDefault(); ghSaveConfirm(); }
});
...
async function mcGenerate() {
  ...
  document.getElementById('mcStatus').innerHTML = '<span style="color:var(--text3)">⏳ الذكاء الاصطناعي يكتب الكود... (20-60 ثانية)</span>';
  const mcCtrl = new AbortController();
  const mcTimer = setTimeout(() => mcCtrl.abort(), 90000);
  try {
